...
public class WinRegistry {
    ...
    private static String _convertByteArrayToHexString(byte [] array) {
        StringBuilder str_bld = new StringBuilder();
        for (byte b : array) {
            str_bld.append(String.format("%02X-", b));
        }
        return str_bld.toString().replaceAll("-$", "");
    }

    private static byte [] _convertHexStringToByteArray(String string) {
        String [] str_array = string.split("-");
        byte [] array = new byte[str_array.length];
        for (int i = 0; i < str_array.length; i++) {
            String str = str_array[i];
            byte value = (byte) (Integer.decode("0x" + str).intValue());
            array[i] = value;
        }
        return array;
    }
